...
>
> **前置依赖**：需已完成因子1且通过。所有参数优先引用 `analysis_input_summary.md` 的锚定值；若摘要不存在，再回退引用模块〇(8)的补充项。
>
...
过去3~5年支付率序列：[X1%, X2%, X3%, X4%, X5%]（来源：analysis_input_summary / data_pack_report P11 历史表）
...
注：
- 分析时应询问用户持股渠道（直接/港股通/持有时长），若用户未指定则默认：港股用港股通税率，A股用长期持有税率
- 注销型回购不做税务折扣
...
```
支付率锚定值（analysis_input_summary 已锚定）= [X]%（M）
  基准 = min(近3年支付率均值, 承诺支付率)
  若存在最新派息信号（DPS较年报提升>15%），取信号推算值与基准的较高者
  注：支付率锚定和派息信号优先取自 analysis_input_summary，必要时回查年报附注原文
