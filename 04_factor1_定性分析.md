...
> 目的：(1) 为后续模块提供背景数据与异常识别；(2) 锚定因子2/3应使用的利润口径；(3) **核对 `analysis_input_summary.md` 中已整合的关键参数是否合理**。
...
> **核心原则：统一输入优先来自 `analysis_input_summary.md`。**
> 本节不再承担默认的数据整合职责，而是对摘要中的关键参数做合理性校验，并仅在摘要缺失字段时做少量补充。
...
过去3年支付率序列 = [X%, X%, X%]         | 来源：[analysis_input_summary §6 / data_pack_report P11 历史表]
支付率均值 [M_hist] = [值]%              | 计算：近3年均值
最新派息信号 = [无变化 / DPS已提升至{值}] | 来源：[analysis_input_summary §6 / data_pack_market §3B]
...
═══ 估值基准参数（→ 因子4）═══
Rf（十年期国债）[Rf] = [值]%       | 来源：[analysis_input_summary §6 / 日期+来源]（A股/港股→中国国债）
...
> **后续引用规则**：因子2/3/4中优先使用 `analysis_input_summary.md` 中的锚定参数。若本节对摘要缺失项做了补充，格式为「参数名 = X（模块〇(8)补充项）」；若需调整（如敏感性分析），必须标注调整原因。
...
第4步：控股折价率 = (SOTP估值 − 母公司市值) / SOTP估值
       母公司市值优先引用 analysis_input_summary 中的市值锚定项
...
传递至因子2/3/4的定量参数（优先引用 analysis_input_summary）：
  市值 = [X] 亿元（analysis_input_summary）
  GAAP归母净利润 = [C] 亿元（analysis_input_summary）
  Non-GAAP归母净利润 = [C'] 亿元（analysis_input_summary / 模块〇补充）
  主营经营利润 = [X] 亿元（analysis_input_summary / 模块〇补充）
  D&A = [D] 亿元 | Capex = [E] 亿元（analysis_input_summary）
  OE预估 = [X] 亿元（analysis_input_summary / 模块〇补充）
  广义净现金 = [X] 亿元（analysis_input_summary）
  狭义净现金 = [X] 亿元（analysis_input_summary）
  年均回购 = [O] 亿元（analysis_input_summary）
  支付率锚定值 = [X]%（analysis_input_summary）
  Rf = [X]% | 门槛 = [X]%（analysis_input_summary）
...
*龟龟投资策略 v0.15 | 因子1 定性分析 | 通用 AI 平台兼容*
