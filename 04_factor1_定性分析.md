...
> - `data_pack_market §3, "营业收入"`
> - `data_pack_report P3, "经营利润"`
> - `业绩公告, "调整后归母净利润"`
> 若同一指标在不同来源中数值不同（如四舍五入差异），以当前 SKILL 约定的口径优先级裁决，并标注差异。
...
> 本节不再重复罗列后续因子所需的全部参数，也不再默认承担跨数据包整合职责。任务只有两项：校验摘要中的关键锚定值是否自洽；在摘要缺失字段时补少量必要说明。
...
校验项A：市值与盈利锚定值是否自洽
  当前股价 / 总股本 / 当前市值 / 汇率
  GAAP归母净利润 / 主营经营利润 / D&A / Capex
  若摘要值与本模块(1)-(4)复盘结果差异 >5%，必须标注差异来源

校验项B：现金与负债口径是否自洽
  现金及等价物 / 短期投资 / 定期存款或理财 / 受限现金
  短期债务 / 长期债务 / 有息负债合计
  狭义净现金 / 广义净现金
  若广义口径包含合同负债，必须单独说明适用理由

合同负债计入类现金的补充判断（仅摘要未裁决时启用）：
  1. 业务模式为预收制或先款后货
  2. 对应交付确定性极高
  3. 近3年余额稳定，非一次性大额
  若不同时满足 → BB_contract = 0，BB_super = BB

校验项C：分配与回购锚定值是否自洽
  最新年报股息总额 / 过去3年支付率序列 / 支付率锚定值 [M]
  最新派息信号 / 注销型回购年均金额 [O]
  若最新DPS信号与年报附注不一致，优先保留摘要中的裁决并注明来源
...
交叉校验指标（仅保留异常识别所需）：
  ...
  Owner Earnings预估 [I] = [值] 亿元 | 若摘要缺失再由本模块补算
```

> **后续引用规则**：因子2/3/4默认直接读取 `analysis_input_summary.md`。本模块仅在摘要缺失时产生「模块〇(8)补充项」，且补充项只限于口径说明、异常解释或少数缺失值。
...
传递至因子2/3/4的定量输入：
  默认输入文件：analysis_input_summary.md
  若本模块存在补充项：逐项写明「字段 = 值（模块〇(8)补充项，原因：xxx）」
  若无补充项：注明「后续因子直接使用摘要锚定值，不再重复取数」
