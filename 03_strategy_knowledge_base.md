...
> 本文件为全局规则库，供协调器、数据整合层、各因子模块和报告模板共同引用。
...
**标准入口**：若存在 `analysis_input_summary.md`，Phase 3 优先读取该摘要文件；若不存在，再回退读取原始数据包。若 `data_pack_report.md` 不存在，标注「⚠️ 无年报PDF数据包，以下分析基于市场数据包」，相关项目使用降级方案。
...
推荐引用方式：
- 协调器：引用本文件中的单位规则、降级规则、分析纪律
- Phase 2.5：引用本文件中的字段引用规范、状态标注规范
- 因子模块：引用本文件中的单位换算、边界条件处理规则
- 报告模板：引用本文件中的数据引用规范与语言规范
